...
// 給中間值（14）。
//
// round 9 這一檔原本沿用表面數字 5，但 Task 4 的 tests/solvability.test.ts 掃過
// 8 個生物 × 40 顆種子後量到 mistfawn round9 seed4 衝到 17 條線索（超過 14 條的
// 可讀性上限）——門檻 5 太接近 age2（起始蹤跡所在齡，線索數天生較多）的真實候選
// 集合大小，收斂迴圈 3 次都補不進去，被迫打滿每齡 3 條的追加上限。10 仍不夠
// （同一顆種子還是衝到 17），12 才讓同一顆種子收斂在 13 條、全部 320 局的上限
// 落在 13。12 已比 round<=7 的 14 更嚴格，符合「難度越高、齡內線索該越集中」的
// 曲線方向，故未進一步放寬到 14。
//
