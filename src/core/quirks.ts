...
//
// 第四次調整（task-4-report.md）：Phase 6a 為了讓可解性達標而動用的兩個全域槓桿
// （route.ts 的 MOVE_EVERY 12→16、SPACING 收緊）代價很大——貪婪追擊實測顯示
// 獵物在 48.7%（1500 局中 730 局）的獵局裡從頭到尾沒換過節點，等於這一階要教
// 玩家的「預測牠會在哪」有將近一半用不上。這裡把記錄說清楚：這兩個槓桿在 bias
// 下修到 0.04 之後仍是必要的——再審時測過把它們還原（MOVE_EVERY=12、原始
// SPACING），ridgecrest r1 的超支率量到 2.40%，超過 owner 核准的 2.33% 臨界，
// 不合格；不是「早就不需要了」。owner 這次核准的方向反過來：把兩個槓桿還原，
// 靠繼續下修 bias 把超支率壓回門檻內。在 MOVE_EVERY=12、原始 SPACING 之下，
// round-1、1000 顆種子（seed = seed*131+round）掃過 0.04（沿用還原前的參照值，
// 超預算率 4.60%）之後再往下掃 0.03/0.02/0.01/0.00（崖壁佔比欄與槓桿無關，
// 是地形場本身在該 bias 下的量測，seed 1..100、size 20 慣例）：
//
//   bias  超預算率  補給後仍無法完成率  崖壁佔比均值  崖壁佔比最糟
//   0.04    4.60%          0.10%              —             —
//   0.03    3.70%          0.10%            3.53%         18.75%
//   0.02    2.90%          0.10%            2.97%         16.50%
//   0.01    2.20%          0.00%            2.50%         15.50%
//   0.00    2.40%          0.00%            2.06%         14.75%
//
// 只有 0.01 同時滿足兩條門檻（≤2.33% 超預算、≤0.67% 無法完成）；0.02 以上超支率
// 已經超標，0.00 反而比 0.01 差（2.40% vs 2.20%，非單調——地形雜訊場在不同
// bias 下位移的方式不是線性的，量測就是這樣）。取 0.01：稜脊獸腳下仍比其他
// 生物略多岩坡與崖壁（崖壁均值 2.50% vs 其餘生物 ~2%），超支率壓到 2.20%、
// 無法完成率壓到 0.00%，同時把 MOVE_EVERY／SPACING 兩個全域槓桿完全還原，
// neverMoved 從 48.7% 打回 12.33%（1500 局貪婪追擊，接近 Phase 6a 之前的
// 12.6% 基準）。這是本次調整的核心取捨：用 ridgecrest 一隻生物的個性強度
// （岩坡崖壁的視覺份量）換回所有生物共享的路線可預測性。terrain.test.ts 的
// G1 測試上界已同步收緊到這個 bias 下的量測值（0.19，見該檔案）。
export function elevationBiasFor(creatureId: string): number {
  return creatureId === 'ridgecrest' ? 0.01 : 0;
}
