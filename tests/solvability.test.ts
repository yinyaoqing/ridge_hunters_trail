...
describe('可解性掃描（規格 §8）', () => {
  ...
  // 不穩地超時，故明訂逾時。
  //
  // 逾時值 20000ms→60000ms：20000 這個數字是照開發機量到的 12.6 秒訂的，從來
  // 沒有對 CI 的硬體驗證過，結果在 GitHub Actions 的 ubuntu-latest（2 核）上
  // 實測 27.7 秒、直接紅燈。量過成本組成：7200 關裡生成佔 5.9 秒、三次 Dijkstra
  // 佔 6.4 秒（開發機），約各半——沒有可以砍掉的重複工作，這支測試在 CI 上就是
  // 要花將近 28 秒，不是它退步了。（唯一看得出的浪費是 fromStart 這張整圖成本表
  // 只被查了 trailhead 一格；但換成單點搜尋最多省下三分之一的 Dijkstra、CI 上
  // 仍要 23 秒左右，而且會動到成本公式，讓上面那幾段用現行公式量出來的可解率
  // 數字全部失效——不划算。）
  // 60000ms 是對 CI 實測值留兩倍餘裕：GitHub 的共用 runner 受鄰居影響，變異
  // 本來就大，一倍餘裕不夠。這個值依然抓得到真正的當機或無窮迴圈。
